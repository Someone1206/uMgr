    ...
    wxFrame(nullptr, wxID_ANY, title, pos, size)
{
    ...
    {
        std::ifstream file(GV::consts::user_data_folder + FSEP + "LastLogs.baka");
...
void mainFrame::allLogsS(wxCommandEvent& evt) {
    if (!_allLog) {
        std::ifstream file(GV::consts::user_data_folder + FSEP + "AllLogs.hentai");
        readTrackerFile(file, LogList, logs);
    }
    _allLog = true;
}
...
void mainFrame::choice(wxCommandEvent& evt)
{
    ...
    if (opt != _prevOpt) { // if current selection != previous selection
        if (opt != std::string(genres[0].mb_str())) { // if != to log summary
            ...
            int len = entriesNGenres((GV::consts::user_data_folder + FSEP + opt), true);
            ...
            if (len != 0) // if no entries present
            {
                ...
                readTrackerFile(emptyFile, Entries, emptyText, 0, entryList, (GV::consts::user_data_folder + FSEP + opt), false, entryFP);
